    ...
    class CurrentWeatherController < ActionController::API
      ...
      def show
        json = if params[:query_type] == 'by_coord'
                 Rails.cache.fetch("current_weather/by_coord/#{params[:lat]}/#{params[:lon]}", expires_in: 10.minutes) do
                   owm_client.geocode(lat: params[:lat], lon: params[:lon])
                   parse_owm_response(owm_client)
                 end
               else
                 Rails.cache.fetch("current_weather/by_city/#{params[:city]}/#{params[:country]}", expires_in: 10.minutes) do
